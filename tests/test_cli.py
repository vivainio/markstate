...
def _setup_flow_with_skill_variable(tmp_path: Path) -> None:
    setup_flow(
        tmp_path,
        "$variables:\n"
        "  skill:\n"
...
def test_vars_clear_removes_all_persisted(tmp_path):
    ...
    result2 = run(["vars"], tmp_path)
    assert result2.returncode == 0
    assert "(none)" in result2.stdout


def test_vars_clear_on_already_empty_reports_none(tmp_path):
    setup_flow(tmp_path)
    result = run(["vars", "--clear"], tmp_path)
    assert result.returncode == 0
    assert "(none)" in result.stdout
...
def test_vars_rejects_unknown_variable(tmp_path):
    setup_flow(tmp_path)
    result = run(["vars", "nope=1"], tmp_path)
    assert result.returncode != 0
    assert "unknown variable 'nope'" in result.stderr
    assert not (tmp_path / ".markstate-variables").exists()
...
def test_vars_rejects_value_outside_declared_values(tmp_path):
    _setup_flow_with_skill_variable(tmp_path)
